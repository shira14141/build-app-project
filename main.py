import json
import uuid
import mimetypes # 🔥 הספריה החדשה שמזהה אם זה PDF או תמונה
...
class TaskResponse(BaseModel):
    id: str; title: str; assigned_to: str; priority: int; status: str

class ExpenseResponse(BaseModel):
    catalog_id: str; title: str; final_price: float

class FileMetaResponse(BaseModel):
    id: str; filename: str; uploaded_by: str

class ProjectResponse(BaseModel):
    id: int; name: str; initial_budget: float; partners: List[str]
    tasks: List[TaskResponse] = []
    expenses: List[ExpenseResponse] = []
    files: List[FileMetaResponse] = []

class PersonalTaskResponse(BaseModel):
    id: str; title: str; assigned_to: str; priority: int; status: str; date: str
...
USERS_DB = {"יוסי הקבלן": "1234", "דנה האדריכלית": "1234", "משה מפקח הבנייה": "1234", "אבי אינסטלטור": "1234", "שירה": "admin", "רוני קבלן חשמל": "1234"}
dekel_catalog = {"101": {"title": "יציקת בטון", "default_price": 350.0}, "102": {"title": "בניית קיר", "default_price": 120.0}, "103": {"title": "נקודת חשמל", "default_price": 250.0}}
...
def create_project(project: dict, db: Session = Depends(get_db)):
    new_proj = DBProject(id=project['id'], name=project['name'], initial_budget=project['initial_budget'], partners=json.dumps(project['partners']))
    db.add(new_proj)
    db.commit()
...
def get_projects(db: Session = Depends(get_db)):
    ...
    for p in db.query(DBProject).all():
        ...
        res.append(ProjectResponse(id=p.id, name=p.name, initial_budget=p.initial_budget, partners=json.loads(p.partners), tasks=tasks, expenses=expenses, files=files))
    return res

@app.delete("/projects/{project_id}")
def delete_project(project_id: int, db: Session = Depends(get_db)):
    db.query(DBProject).filter(DBProject.id == project_id).delete(); db.commit()
    return {"message": "Deleted"}

@app.post("/projects/{project_id}/tasks/")
def add_task(project_id: int, task: dict, db: Session = Depends(get_db)):
    db.add(DBTask(id=str(uuid.uuid4()), project_id=project_id, title=task['title'], assigned_to=task['assigned_to'], priority=task['priority']))
    db.commit()
    return {"message": "Added"}

@app.delete("/projects/{project_id}/tasks/{task_id}")
def delete_task(project_id: int, task_id: str, db: Session = Depends(get_db)):
    db.query(DBTask).filter(DBTask.id == task_id).delete(); db.commit()
    return {"message": "Deleted"}
...
def update_task_status(project_id: int, task_id: str, new_status: str, db: Session = Depends(get_db)):
    db.query(DBTask).filter(DBTask.id == task_id).first().status = new_status; db.commit()
    return {"message": "Updated"}

@app.post("/projects/{project_id}/expenses/")
def add_expense(project_id: int, expense: dict, db: Session = Depends(get_db)):
    price = expense['custom_price'] if expense['custom_price'] is not None else dekel_catalog[expense['catalog_id']]["default_price"]
    db.add(DBExpense(project_id=project_id, catalog_id=expense['catalog_id'], title=dekel_catalog[expense['catalog_id']]["title"], final_price=price))
    db.commit()
    return {"message": "Added"}

# --- ניהול קבצים ---
@app.post("/projects/{project_id}/files/")
def upload_file(project_id: int, file: UploadFile = File(...), uploaded_by: str = Form(...), db: Session = Depends(get_db)):
    db.add(DBFile(id=str(uuid.uuid4()), project_id=project_id, filename=file.filename, uploaded_by=uploaded_by, data=file.file.read()))
    db.commit()
    return {"message": "File uploaded"}

# 🔥 הנתיב החדש שלנו: פתיחת קובץ לצפייה (View)
...
def view_file(file_id: str, db: Session = Depends(get_db)):
    ...
    # ננסה לזהות אם זה PDF, תמונה וכו'
    mime_type, _ = mimetypes.guess_type(db_file.filename)
    if not mime_type: mime_type = "application/octet-stream"
    # שימי לב למילה 'inline' שאומרת לדפדפן להציג את זה במסך!
    return Response(content=db_file.data, media_type=mime_type, headers={"Content-Disposition": f'inline; filename="{db_file.filename}"'})

# נתיב ההורדה הקיים (Download)
@app.get("/files/{file_id}/download")
def download_file(file_id: str, db: Session = Depends(get_db)):
    db_file = db.query(DBFile).filter(DBFile.id == file_id).first()
    if not db_file: raise HTTPException(status_code=404)
    return Response(content=db_file.data, media_type="application/octet-stream", headers={"Content-Disposition": f'attachment; filename="{db_file.filename}"'})

@app.delete("/files/{file_id}")
def delete_file(file_id: str, db: Session = Depends(get_db)):
    db.query(DBFile).filter(DBFile.id == file_id).delete(); db.commit()
    return {"message": "Deleted"}
...
def get_personal_tasks(user: str, db: Session = Depends(get_db)):
    return [PersonalTaskResponse(**t.__dict__) for t in db.query(DBPersonalTask).filter(DBPersonalTask.assigned_to == user).all()]
...
def delete_pt(task_id: str, db: Session = Depends(get_db)):
    db.query(DBPersonalTask).filter(DBPersonalTask.id == task_id).delete(); db.commit()
    return {"message": "Deleted"}